...
Among the four kernels in the warm-up, each have different characteristics regarding temporal and spatial locality and thus, whether they will be compute or memory bound, so the optimization methods might change for each.

 Y[j] += Y[j] + A[j][i] * B[i] -- memory bound -- 0.09 FLOPs/byte

 s += A[i] * A[i] -- compute bound -- 0.23 FLOPs/byte
 
 s += A[i] * B[i] -- memory bound -- 0.125 FLOPs/byte 

 Y[i] = A[i] + C*B[i] -- also memory bound -- 0.0833 FLOPs/byte
